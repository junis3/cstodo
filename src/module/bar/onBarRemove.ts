  ...
  for (const s of command.slice(1).join(' ').split(',')) {
    ...
    if (!isInteger(content)) {
      await replyDdokddul(
        event,
        user,
        '뺄 일의 번호를 알려주세요... 내용을 알려주시면 똑떨이에요...'
      );
      return;
    }
    ...
    if (x <= 0 || x > bars.length) {
      await replyDdokddul(
        event,
        user,
        `할 일이 ${bars.length}개인데 여기서 ${x}번째 할 일을 빼면 똑떨이에요...`
      );
      return;
    }
  ...
  for (const content of Array.from(contents)) {
    if (await removeBar({ owner: user.id, content })) {
      await replySuccess(
        event,
        user,
        `${user.name}님의 진행중인 일에서 *${content}* 를 제거했어요!`,
        'remove',
        {
          forceMuteType:
      ...
      await replyFail(
        event,
        user,
        `${user.name}님의 진행중인 일에서 *${content}* 를 제거하는 데 실패했어요...`
      );
    }
  }
};
