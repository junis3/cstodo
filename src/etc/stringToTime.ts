...
const stringToTime = (str: string) => {
  const now = new Date();
  let year = now.getFullYear(); let month = now.getMonth(); let date = now.getDate(); let hour: number | null = null; let minute = 0; let
    second = 0;
  let isAfternoon: boolean | undefined;
  ...
  if (year === now.getFullYear() && month === now.getMonth() && date === now.getDate()) {
    if (isAfternoon === undefined && hour > 0 && hour < 12 && hour <= now.getHours() && minute === 0 && second === 0) {
      hour += 12;
    }
  }
