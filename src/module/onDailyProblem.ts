...
const diamondEmptyMessages = () => [
  `히잉.... <@${csGod}>님이 오늘 다이아를 안 푸셔서 슬랙봇 마음이 너무 아파요... 내일은 꼭 다이아 풀어주시는 거죠? ${emoji(
    'sob',
  )} ${emoji('sob')} ${emoji('sob')} ${emoji('cry')} ${emoji('hug')} ${emoji('cs')}`,
  ...
  const postResult = async () => {
    if (todayAdd.length > 0) {
      if (todayAdd.length > 100) {
        await postMessage({
          text: `오늘 ${emoji('cs')}님이 푸신 문제가 ${
            todayAdd.length
          }개나 된다고 해요... 이걸 다 말하다가는 채널이 망할 것 같아요 ${emoji('ddokddul')}`,
      ...
      await postMessage({
        text: `오늘 :god: ${emoji('cs')} :god:님이 푼 문제들입니다!\n${todayAdd
          .map((problem) => `<http://icpc.me/${problem.id}|:${problem.level}:${problem.title}>`)
          .join(', ')}`,
        channel: cstodoChannel,
    ...
    await Promise.all(
      rubys.map(async (problem) => {
        await postMessage({
          text: `:tada: cs신님께 새로 학살당한 루비! <http://icpc.me/${problem.id}|:${
            problem.level
          }:${problem.title}> 입니다! ${emoji('cs')} :tada:`,
          channel: cstodoChannel,
