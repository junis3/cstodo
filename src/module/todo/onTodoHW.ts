import {
  getLatestGreenGolds, greenGoldToHrefNoLevel, greenGoldToPlainText, GreenGoldType, removeGreenGold,
...
import {
  getUser, setHWInitialTime, setHWQuery, setHWRepeatTime, setNumProbsPerCycle,
} from '../../database/user';
...
const onTodoHWPurge: TodoRouter = async ({ event, user }) => {
  ...
  const removalResult = await Promise.all(solvedHW.map(async (hw) => (!!(await removeGreenGold(hw)))));
...
const onTodoHWTime: TodoRouter = async ({ event, user }) => {
  ...
  return new ReplySuccessCommand(
    event,
    user,
    `${user.name}님의 다음 숙제는 ${timeToString(nextTime)}에 드리겠습니다! 지금은 ${repeatTime}일마다 드리고 있어요.`,
  );
};
...
const onTodoHWSet: TodoRouter = async ({ query: { args, rawArgString }, event, user }) => {
  ...
  if (typeof numProblemsArg === 'string') {
    ...
    if (Number.isNaN(numProblems) || numProblems < 1 || numProblems > 10) {
      return new ReplyFailureCommand(event, user, '숙제 개수는 1개 - 10개 사이의 양의 정수로 입력해주세요.');
    }
