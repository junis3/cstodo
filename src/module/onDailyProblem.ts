...
const diamondEmptyMessages = () => [
  `히잉.... <@${csGod}>님이 오늘 다이아를 안 푸셔서 슬랙봇 마음이 너무 아파요... 내일은 꼭 다이아 풀어주시는 거죠? ${emoji(
    'sob'
  )} ${emoji('sob')} ${emoji('sob')} ${emoji('cry')} ${emoji('hug')} ${emoji(
    'cs'
  )}`,
...
const emptyMessages = () =>
  [
    ...
    `저 오늘 우체국 가서 싸우고 왔어요... <@${csGod}>님의 PS 실력을 박스에 담아서 부치려고 했더니 그렇게 큰 박스는 없다는 거 있죠....... 내일은 실력 보여주실 수 있으시죠? ${emoji(
      'cry'
    )} ${emoji('hug')} ${emoji('cs')}`,
...
const dailyProblem = async () => {
  ...
  const todayRemove = history.filter(
    (history) => !currentHistory.find((id) => history.id === id)
  );
  ...
  await Promise.all(
    todayRemove.map(async (history) => removeHistory(history.id))
  );
  await Promise.all(
    todayAdd.map(async (history) =>
      addHistory({
        id: history.id,
        title: history.title,
  ...
  const postResult = async () => {
    if (todayAdd.length > 0) {
      if (todayAdd.length > 100) {
        await postMessage({
          text: `오늘 ${emoji('cs')}님이 푸신 문제가 ${
            todayAdd.length
          }개나 된다고 해요... 이걸 다 말하다가는 채널이 망할 것 같아요 ${emoji(
            'ddokddul'
          )}`,
    ...
    await Promise.all(
      rubys.map(async (problem) => {
        await postMessage({
          text: `:tada: cs신님께 새로 학살당한 루비! <http://icpc.me/${
            problem.id
          }|:${problem.level}:${problem.title}> 입니다! ${emoji('cs')} :tada:`,
          channel: cstodoChannel,
  ...
  if (
    Math.random() < 0.15 ||
    (diamonds.length === 0 && rubys.length === 0) ||
    rubys.length > 0
  ) {
    await postMessage({
