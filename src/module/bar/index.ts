...
const onBar = async (event: SlackMessageEvent, user: UserType) => {
  ...
  if (query.command[0] === 'edit' || query.command[0] === 'update') await onBarEdit(query, event, user);

  if (query.command[0] === 'add' || query.command[0] === 'push' || query.command[0] === 'append') { await onBarAdd(query, event, user); }

  if (query.command[0] === 'remove' || query.command[0] === 'delete' || query.command[0] === 'erase') { await onBarRemove(query, event, user); }
