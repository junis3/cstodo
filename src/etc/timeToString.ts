const weekdayString = (date: Date) => ['일요일', '월요일', '화요일', '수요일', '목요일', '금요일', '토요일'][date.getDay()];

const timeToString = (timestamp: number) => {
  const time = new Date(timestamp); const
    now = new Date();

  let dateResult = (() => {
    const timeDay = Math.floor(time.getTime() / (24 * 60 * 60 * 1000) - now.getTimezoneOffset() / 24 / 60 + 1e-11);
    const nowDay = Math.floor(now.getTime() / (24 * 60 * 60 * 1000) - now.getTimezoneOffset() / 24 / 60 + 1e-11);
  ...
  const timeResult = (() => {
    ...
    } return `${hourToString(time.getHours())}:${format(time.getMinutes())}:${format(time.getSeconds())}`;
  })();
